...
【分类前必须检查】：
在判断是否为政策文件信息（类型3）之前，请务必执行以下检查：
1. 仔细搜索文档内容和文件名，查找是否包含完整的发文文号
   - 发文文号必须包含：发文单位简称+年份（〔〕或[]括起来）+编号+号
   - 例如：津财农〔2024〕95号、洛碛人大〔2019〕4号
   - 注意：单独的日期（如2024年、2024-12-25）、单独的编号（如第95号）、只有年份和编号但没有单位简称的，都不是发文文号
2. 如果找不到完整的发文文号（缺少单位简称、年份、编号或"号"字中的任何一项），无论内容多么像政策文件，都必须归类为"办文材料信息"（类型2）
3. 只有明确找到完整的发文文号（包含单位简称+年份+编号+号）且内容涉及政策，才能归类为类型3
